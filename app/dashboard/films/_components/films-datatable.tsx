...
export const columns: ColumnDef<IFilm>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const rating10 = row.original.rating.avarage;
      const rating5 = rating10 / 2;

      return (
        <div className="text-left flex items-center justify-start gap-2 ml-4">
          {rating10.toFixed(1)}
          <ReactStars count={5} value={rating5} edit={false} size={20} />
        </div>
      );
    },
  },
  ...
  {
    accessorKey: "meta.likes",
    header: "Likes",
    cell: ({ row }) => (
      <div className="text-left">{row.original.meta.likes}</div>
    ),
  },
  {
    ...
    header: "Published",
    cell: ({ row }) => (
      <div className="text-left">{row.original.published ? "Yes" : "No"}</div>
    ),
  },
  {
    accessorKey: "genres",
